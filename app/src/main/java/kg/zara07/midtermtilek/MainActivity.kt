...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.beginTransaction().replace(R.id.navigation,Explore()).commit()
        // now create a menu
    ...
    val bottomListener = BottomNavigationView.OnNavigationItemSelectedListener {
        ...
        when(it.itemId){
            R.id.home -> {
                currentFragment = Explore()
            }
            R.id.home -> {
                currentFragment = Home()
            }
        }
        supportFragmentManager.beginTransaction().replace(R.id.navigation,currentFragment).commit()
        true
    }
}